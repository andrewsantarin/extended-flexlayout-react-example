...
import * as FlexLayout from "@andrewsantarin/extended-flexlayout-react";
import {
  Node,
  TabSetNode,
  TabNode,
  DropInfo,
  BorderNode
} from "@andrewsantarin/extended-flexlayout-react";
...
export default class App extends React.Component<
  any,
  { layoutFile: string | null; model: FlexLayout.Model | null; adding: boolean }
> {
  ...
  loadLayout(layoutName: string, reload?: boolean) {
    ...
    let loaded = false;
    if (!reload) {
      var jsonText = localStorage.getItem(layoutName);
    ...
    if (!loaded) {
      Utils.importFile(
        // I don't know how to dynamically require() a file yet...
        layoutName,
        this.load.bind(this),
      ...
      // Utils.importFile(
      //   // layoutName,
      //   "../layouts/" + layoutName + ".layout.json",
      //   this.load.bind(this),
      //   this.error.bind(this)
      // );
      // Utils.downloadFile(
  ...
  error(reason: string) {
    alert(
      "Error loading json config file: " +
        this.loadingLayoutName +
        "\n" +
        reason
    );
  }
  ...
  onAddClick(event: Event) {
    if (this.state.model!.getMaximizedTabset() == null) {
      (this.refs.layout as FlexLayout.Layout).addTabWithDragAndDropIndirect(
  ...
  onAddClickCustomSize(event: Event) {
    if (this.state.model!.getMaximizedTabset() == null) {
      (this.refs.layout as FlexLayout.Layout).addTabWithDragAndDropIndirect(
        "Add (600 x 200)px grid if free floating<br>(Drag to location)",
        {
          component: "grid",
          name: "a new grid",
          config: {
  ...
  factory(node: TabNode) {
    ...
    if (component === "grid") {
      ...
      return (
        <div className="floating__tab_content">
          <SimpleTable
            fields={fields}
      ...
      return (
        <FlexLayout.Layout model={model} factory={this.factory.bind(this)} />
      );
    } else if (component === "text") {
      return (
        <div dangerouslySetInnerHTML={{ __html: node.getConfig().text }} />
      );
    }
  ...
  onThemeChange(event: React.FormEvent) {
    var target = event.target as HTMLSelectElement;
    let flexlayout_stylesheet: any = window.document.getElementById(
      "flexlayout-stylesheet"
    );
    let index = flexlayout_stylesheet.href.lastIndexOf("/");
    let newAddress = flexlayout_stylesheet.href.substr(0, index);
    flexlayout_stylesheet.setAttribute(
      "href",
      newAddress + "/" + target.value + ".css"
    );
    ...
    return (
      <div className="app">
        <div className="toolbar">
          ...
          <button onClick={this.onReloadFromFile.bind(this)}>
            reload from file
          </button>
          <button
            disabled={this.state.adding}
            style={{ float: "right" }}
            onClick={this.onAddClickCustomSize.bind(this)}
          >
            Add (Component-Specific Tabset Size)
          </button>
          <button
            disabled={this.state.adding}
            style={{ float: "right" }}
          ...
          <button
            style={{ float: "right" }}
            onClick={this.onShowLayoutClick.bind(this)}
          >
